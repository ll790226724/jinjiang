module.exports = {
  ...
  props: {
    ...
    $url: '`/v1/components/f6b74ddd-39de-493f-84ab-9d87fcf23fee/data?start=${craneStates.filterRange[0]}&end=${craneStates.filterRange[1]}`',
    method: 'get',
    $style: {
      width: '1300px',
      height: '282px'
  ...
  children: [
    {
      ...
      props: {
        $options: {
          ...
          $series: "[{type: 'line', name: '接件（件）', symbolSize: 8, lineStyle: {width: 3}, yAxisIndex: 0, data: results.map(result => (result[1]))}, {type: 'line', name: '同上月增长', symbolSize: 8, lineStyle: {width: 3}, yAxisIndex: 1, data: results.map(result => (result[3]))}]"
        },
      }
    }
  ]

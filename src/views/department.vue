  ...
  <div class="department">
    ...
    <div class="carousel-item" :style="{transform: `translateX(${1920 * (1 - craneStates.pageIndex)}px)`}">
      <div :style="{height: '955px', width: '1300px', background: 'linear-gradient(180deg, #c0d5e8, rgba(27,116,239, 0))', position: 'absolute', top: '65px', left: '319px'}">
        ...
        <data-loader v-slot="{ results: results }" :url="`/v1/components/05b74ddd-39de-493f-84ab-9d87fcf23fee/data?start=${craneStates.filterRange[0]}&end=${craneStates.filterRange[1]}&department=${craneStates.department}`" method="get" :data="[{label: '投诉性质', amount: 12}]" :style="{width: '860px', height: '369px', position: 'absolute', top: '91px', left: '101px'}">
        ...
        <data-loader v-slot="{ results: results }" :url="`/v1/components/10b74ddd-39de-493f-84ab-9d87fcf23fee/data?start=${craneStates.filterRange[0]}&end=${craneStates.filterRange[1]}&department=${craneStates.department}`" method="get" :data="[{label: '投诉类型', amount: 12}]" :style="{width: '1200px', height: '370px', position: 'absolute', top: '584px', left: '41px'}">
    ...
    <div ref="page_two" class="carousel-item" :style="{transform: `translateX(${1920 * (2 - craneStates.pageIndex)}px)`}">
      ...
      <data-loader ref="page-2-event-source" v-slot="{ results: results }" :url="`/v1/components/09b74ddd-39de-493f-84ab-9d87fcf23fee/data?start=${craneStates.filterRange[0]}&end=${craneStates.filterRange[1]}&department=${craneStates.department}`" method="get" :data="[{label: '事件来源', amount: 12}]" :style="{width: '649px', height: '553px', position: 'absolute', top: '254px', left: '960px'}">
        <donut ref="event-ranking-content" v-if="results" :data="results.map(item => { return {label: item[1], amount: item[0] } } )" labelKey="label" valueKey="amount" :percentage="true" :hideLabel="true" :theme="{background: 'transparent', colors: ['#1B74EF', '#15C689', '#FFBA08', '#BB4430'], whitespace: 'nowrap'}" :legendOptions="{size: '150px', align: ['start', 'center'], layout: 'vertical', label: {fill: '#2E2E2E', size: 20}, position: 'bottom', offset: [0, 78]}" :tooltip="{text: {align: 'center', baseline: 'middle', fill: '#FFFFFF', size: 14, weight: 400}, notation: {fill: '#007AFE', name: 'circle-small', size: 14}}" :tooltipOptions="{background: 'rgba(60, 71, 89, 0.9)', text: {align: 'center', baseline: 'middle', fill: '#FFFFFF', size: 14, weight: 400}, title: {align: 'center', baseline: 'middle', fill: '#FFFFFF', size: 14, weight: 400}}" />
      </data-loader>
    </div>
    <div ref="page_three" class="carousel-item" :style="{transform: `translateX(${1920 * (3 - craneStates.pageIndex)}px)`}">
      ...
      <data-loader v-slot="{ results: results }" :url="`/v1/components/06b74ddd-39de-493f-84ab-9d87fcf23fee/data?start=${craneStates.filterRange[0]}&end=${craneStates.filterRange[1]}&department=${craneStates.department}`" method="get" :style="{width: '1500px', height: '730px', position: 'absolute', top: '181px', left: '210px'}">
        <v-chart v-if="results" :options="{tooltip: {trigger: 'axis', backgroundColor: '#ffffff', textStyle: {color: '#2e2e2e', fontSize: '26px'}, formatter: formatterFunc}, legend: {data: Object.keys(craneStates.chartLegendsMap).map((item) => {return craneStates.chartLegendsMap[item].name}), right: '80px', width: '736px', textStyle: {fontSize: '18px'}}, color: ['#15c689','#bb4430', '#ffba08', '#a2aebb', '#1b74ef','#74797f'], xAxis: [{type: 'category', axisLabel: {rotate: 20, interval: 0, color: '#2e2e2e', fontSize: '18px'}, data: results.map((item) => {return {date: item[0].slice(0, 10), satisfied: item[1], unsatisfied: item[2], basicly: item[3], unknown: item[4], callbacked: item[5], day: item[6].toFixed(2)} }).map((item) => {return item.date}), axisLine: {show: false}, axisTick: {show: false}, splitLine: {show: false}}], yAxis: [{type: 'value', axisLabel: { color: '#2e2e2e', fontSize: '16px'}, scale: true, name: '件', axisLine: {show: false}, axisTick: {show: false}, splitLine: {show: false}}, {type: 'value', scale: true, name: '天', boundaryGap: [0.2, 0.2], axisLine: {show: false}, axisTick: {show: false}, splitLine: {show: false}}], grid: {bottom: 110}, dataZoom: {type: 'slider', show: 'true', xAxisIndex: [0], handleStyle: {color: '#1b74ef'}, startValue: 0, endValue: 19}, series: Object.keys(craneStates.chartLegendsMap).map(item => {return {...craneStates.chartLegendsMap[item], data: results.map((item) => {return {date: item[0], satisfied: item[1], unsatisfied: item[2], basicly: item[3], unknown: item[4], callbacked: item[5], day:  parseFloat(item[6]).toFixed(2) } }).map(data => data[item])}})}" />
      </data-loader>
    </div>
    <div ref="page_four" class="carousel-item" :style="{transform: `translateX(${1920 * (4 - craneStates.pageIndex)}px)`}">
      ...
      <data-loader ref="jiejianqushi-month" v-slot="{ results: results }" v-if="!craneStates.showDayChart" :url="`/v1/components/f7b74ddd-39de-493f-84ab-9d87fcf23fee/data?start=${craneStates.filterRange[0]}&end=${craneStates.filterRange[1]}&department=${craneStates.department}`" method="get" :style="{width: '1500px', height: '708px', position: 'absolute', top: '199px', left: '210px'}">
      ...
      <data-loader ref="jiejianqushi-day" v-slot="{ results: results }" v-if="craneStates.showDayChart" :url="`/v1/components/07b74ddd-39de-493f-84ab-9d87fcf23fee/data?start=${craneStates.filterRange[0]}&end=${craneStates.filterRange[1]}&format=${craneStates.chartFormat}&department=${craneStates.department}`" method="get" :style="{width: '1500px', height: '708px', position: 'absolute', top: '199px', left: '210px'}">

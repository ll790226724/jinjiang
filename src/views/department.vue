<template>
  <div class="department">
    ...
    <data-loader ref="deal-number" v-slot="{ results: results }" v-if="results" :url="`/v1/components/01b74ddd-39de-493f-84ab-9d87fcf23fee/data?start=${craneStates.filterRange[0]}&end=${craneStates.filterRange[1]}&department=${craneStates.department}`" method="get" :data="[[0]]" :style="{width: '194px', height: '44px', position: 'absolute', top: '61px', left: '63px'}">
    ...
    <data-loader ref="percentage-number" v-slot="{ results: results }" :url="`/v1/components/02b74ddd-39de-493f-84ab-9d87fcf23fee/data?start=${craneStates.chainParamsNew}&end=${craneStates.chainParamsOld}&department=${craneStates.department}`" method="get" :data="[[0]]" :style="{width: '70px', height: '22px', boxSizing: 'border-box', color: '#FFFFFF', fontSize: '16px', paddingLeft: '4px', paddingRight: '6px', backgroundColor: '#155EC2', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderRadius: '2px', position: 'absolute', top: '80px', left: '259px'}">
      <brick-tooltip v-if="results" content="窗口办理量下降2%" placement="bottom-right" :style="{borderRadius: '4px', color: '#ffffff', fontFamily: 'Oswald-Light', lineHeight: '1', alignItems: 'center', display: 'flex', flexGrow: '1', justifyContent: 'space-between'}">
